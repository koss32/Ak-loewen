...
test('Ukrainian and Turkish bookings localize preview, status and controls',async()=>{for(const [locale,program,group,consent,adult,participant,status,comment] of [['uk','Бокс','15+ років','Я погоджуюся','Дорослий учасник','Ім’я учасника','Статус','Коментар'],['tr','Boks','15+ yaş','Kabul ediyorum','Yetişkin katılımcı','Katılımcının adı','Durum','Yorum']]){const store=createMemoryBotStore(),bot=createTelegramBot({store,config:cfg});let i=1;await bot.handle(msg(i++,20,'/language'));await bot.handle(cb(i++,20,await button(store,locale==='uk'?'Українська':'Türkçe')));await bot.handle(msg(i++,20,'/help'));assert.match((await last(store)).text,new RegExp(locale==='uk'?'Як надіслати':'Talep nasıl'));await bot.handle(msg(i++,20,'/book'));await bot.handle(cb(i++,20,await button(store,program)));await bot.handle(cb(i++,20,await button(store,group)));await bot.handle(cb(i++,20,await button(store,consent)));await bot.handle(cb(i++,20,await button(store,adult)));await bot.handle(msg(i++,20,'@test_contact'));await bot.handle(msg(i++,20,participant));await bot.handle(msg(i++,20,'20'));await bot.handle(cb(i++,20,await button(store,locale==='uk'?'Понеділок, Середа, П’ятниця · 18:30–20:00 · Europe/Berlin':'Pazartesi, Çarşamba, Cuma · 18:30–20:00 · Europe/Berlin')));await bot.handle(msg(i++,20,'/skip'));assert.match((await last(store)).text,new RegExp(comment));const choices=(await last(store)).meta.reply_markup.inline_keyboard.flat();assert.deepEqual(choices.map(b=>b.text),[botCopy[locale].submit]);assert.equal(Object.keys((await store.inspect()).requests).length,0);await bot.handle(cb(i++,20,await button(store,botCopy[locale].submit)));const records=Object.values((await store.inspect()).requests);assert.equal(records.length,1);assert.equal(records[0].locale,locale);assert.equal(records[0].contactName,'@test_contact');assert.equal(records[0].reminders.enabled,false);assert.equal(records[0].consentVersion,legal.consentVersion);await bot.handle(msg(i,20,'/status'));assert.match((await last(store)).text,new RegExp(status));}});
